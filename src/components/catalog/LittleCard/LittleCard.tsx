...
function LittleCard({ product }: LittleCardProps) {
    ...
    <div className={cx('little-card__wrapper')}>
      ...
      <div className={cx('little-card__info')}>
        <div className={cx('little-card__description')}>
          <p className={cx('little-card__brand')}>{`${product.brand} (${product.manufacturer})`}</p>
        ...
        <div className={cx('little-card__bottom', `${activeClass}`)}>
          <p className={cx('little-card__rating')}>{`Rating:  ${(rating / reviewsQuantity).toFixed(
            1
          )} for ${reviewsQuantity} buyers`}</p>
          <p data-testid="test-price" className={cx('little-card__price')}>{`${product.price}$`}</p>
        </div>
      </div>
    </div>
  );
